...
export class ProductsDataSource implements DataSource<Product> {
  ...
  loadPagedData(queryString = "", withInternalCodeSelector = false, pageIndex = 1, pageSize = 10): any {
    this.loadingSubject.next(true);
      console.log("pageIndex: " +pageIndex);
    this.api.getProducts(queryString, withInternalCodeSelector, pageIndex, pageSize)
      .pipe(
        map(res => {
          return res.body;
        }),
